...
object OniText {
  val oop = """
    Object oriented programming is a way to develop
    software that is based on objects or things.  There are typically
    five principles that define object oriented software.
    more
  """
  val fp = """
    Functional programming is utilizing a programming language
    that supports the notion that a function is a first class
    feature in the language.
  """
  val fc = """
    First class language features are constructs that are naturally
    supported in a programming language.  In an object oriented
    language, classes and objects are first class language constructs.
  """
  val aop = """
    Aspect oriented programming is developing software with the notion
    of aspects in the design or development process. An aspect is something
    outside of an entity that describes it's behavior or structure.
  """
  val sec = """
    Security is a critical component of all systems.
    We have the experience to architect systems so that
    they are not vulnerable to a wide variety of attacks
    Establishing secure best practices is critical in
    protecting data and systems.
  """
  val oni = """
    Object Nirvana, Inc. offers software development
    services in the Java, Scala, and web technologies.
    We bring long experience to support software development,
    architecture, practices, and mentoring.
  """
}
...
class BindingDemoView(model: Property[String]) extends View {
  ...
  private val content = {
    ...
    div(
      ...
      div(OniStyles.blockOfText)("a 123", onclick := mydivf _),
      p("other text 1"),
      div(OniStyles.blockOfText)("a 456", onclick := mydivf _),
      p("other text 2")
    ),
    // p(LocalStyles.obliqueOnHover)("Hover me!"),
    a(DemoStyles.underlineLinkBlack)(href := "http://guide.udash.io/#/frontend/bindings", target := "_blank")(
        "Read more in Udash Guide."),
    div(OniStyles.cubeContainer)(
      div(id:="box", `class`:="show-frontSide")(
    ...
    div( id:="options") (
      ...
      p(
        button( id:="toggle-backface-visibility")("Toggle Backface Visibility")
      )
    ),
    script("""
  //alert('step 0');
var init = function() {
  //alert('step 1');
  var box = document.querySelector('.OniStyles-cubeContainer').children[0],
      //alert('step 2');
...
window.DDD = DDD;
  ...
  def startSpin(): Unit = {
    ...
    setTimeout(5000) { // note the absence of () =>
      nextButton
      startSpin()
    }
  }
...
object DefineFoo {
  def apply(): Unit = () // just trigger the constructor, once

  import js.Dynamic.{global => g}

//  private val XFooProto: js.Dynamic = {
//    val pro1: Object = g.HTMLElement.prototype
//    val new_o = js.Object.create(pro1)
//    new_o.asInstanceOf[js.Dynamic]
//  }

//  XFooProto.foo = { (self: XFoo) =>
//    dom.alert("foo called")
//  }: js.ThisFunction

//  private val XFoo = dom.document.registerElement("x-foo",
//    js.Dynamic.literal(prototype = XFooProto))

  // Store XFoo in the global namespace so that 'new XFoo' can work later
//  g.XFoo = XFoo
}

//class XFoo extends org.scalajs.dom.HTMLElement {
//  def foo(): Unit
//}

//val xfoo = new XFoo
//dom.document.body.appendChild(xfoo)
//
//Element.prototype.hasClassName = function (a) {
//    return new RegExp("(?:^|\\s+)" + a + "(?:\\s+|$)").test(this.className);
//};
